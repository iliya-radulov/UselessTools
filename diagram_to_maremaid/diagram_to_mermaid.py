...
class DiagramApp:
    ...
    def setup_ui(self):
        ...
        # tk.Button's bg color is ignored on some platforms/themes (notably
        # macOS' native theme), and its size was in text-cell units so it
        # didn't reliably fit the narrow sidebar. Small colored rectangles
        # drawn on a Canvas always render their fill color everywhere, and
        # can be sized in exact pixels.
        swatch = 22
        gap = 5
        cols = 6
        rows = math.ceil(len(colors) / cols)
        grid_w = cols * swatch + (cols - 1) * gap
        grid_h = rows * swatch + (rows - 1) * gap
        
        self.color_canvas = tk.Canvas(color_frame, width=grid_w, height=grid_h, highlightthickness=0)
        self.color_canvas.pack()
        
        self.color_swatch_ids = {}   # canvas rect id -> color hex
        self.color_swatch_rects = [] # list of (rect_id, color) for redrawing selection ring
        for idx, (name, color) in enumerate(colors):
            r, c = divmod(idx, cols)
            x0 = c * (swatch + gap)
            y0 = r * (swatch + gap)
            x1, y1 = x0 + swatch, y0 + swatch
            rect_id = self.color_canvas.create_rectangle(
                x0, y0, x1, y1, fill=color, outline="#888888", width=1
            )
            self.color_canvas.tag_bind(rect_id, "<Button-1>", lambda e, c=color, rid=rect_id: self.select_color(c, rid))
            self.color_swatch_ids[rect_id] = color
            self.color_swatch_rects.append((rect_id, color))
        
        # mark the default color as selected
        for rect_id, color in self.color_swatch_rects:
            if color == self.shape_color.get():
                self.color_canvas.itemconfig(rect_id, outline="#2c3e50", width=3)
                break
    ...
    def select_color(self, color, rect_id):
        """Called when a color swatch is clicked - set active color and show which one is selected"""
        self.shape_color.set(color)
        for rid, _ in self.color_swatch_rects:
            self.color_canvas.itemconfig(rid, outline="#888888", width=1)
        self.color_canvas.itemconfig(rect_id, outline="#2c3e50", width=3)
            ...
            if item:
                ...
                for shape_data in self.shapes:
                    if any(part == item[0] for part in shape_data["parts"]):
                        ...
                        # Canvas needs keyboard focus or the <Delete> binding never fires
    ...
    def on_canvas_release(self, event):
        if self.selected_shape:
            # Keep the shape selected after a move (or a plain click) so
            # Delete / Delete Selected have something to act on. Selection
            # is only cleared by clicking elsewhere (see on_canvas_click).
            self.status_bar.config(
                text=f"Selected: {self.selected_shape['type']} (ID: {self.selected_shape['id']}) — press Delete to remove"
            )
            return
    ...
    def contrast_text_color(self, hex_color):
        """Pick black or white text for legibility against a given fill color"""
        h = hex_color.lstrip('#')
        if len(h) != 6:
            return "#000000"
        try:
    ...
    def generate_mermaid(self):
        ...
        # Explicit per-node colors — without this, Mermaid falls back to
        # the viewer's default theme, which is exactly why nodes can look
        # like "empty boxes" under a dark-mode renderer (default text color
        # ends up with poor/no contrast against the default fill). Writing
        # the fill AND a contrast-matched text color makes each node render
        # consistently regardless of the surrounding theme.
        if node_ids:
            mermaid_code += "\n    %% Colors\n"
            for shape, node_id in zip(node_shapes, node_ids):
                fill = shape.get("color") or "#3498db"
                text_color = self.contrast_text_color(fill)
                mermaid_code += f"    style {node_id} fill:{fill},stroke:#2c3e50,stroke-width:2px,color:{text_color}\n"
