...
Issue 5: Cyrillic/non-Latin text shows as black boxes (■■■) in the PDF report

This happened in two places, both in `generate_pdf_report()`:

1. **Font never registered at all.** The font-lookup code called `os.path.exists(...)`
   but the file never did `import os` — so registration silently threw a
   `NameError` (swallowed by a bare `except`), fell back to Helvetica, and
   Helvetica has no Cyrillic/Greek glyphs.

   Fix: add `import os` at the top of the file, and — more reliably than
   hunting for system fonts — pull the Unicode font straight from
   matplotlib's own bundled DejaVu Sans, since matplotlib is already a
   dependency and DejaVu Sans covers Cyrillic, Greek and Latin-Extended:
   ...
   System font paths (`/usr/share/fonts/...`, `C:/Windows/Fonts/...`, etc.)
   are kept as a secondary fallback in case matplotlib isn't available.

2. **Font applied to the header row only, not the data rows.** In the
   `TableStyle` calls, the font override was scoped like this:

   ```python
   ('FONTNAME', (0, 0), (-1, 0), font_name),   # (-1, 0) = last col, ROW 0 ONLY
   ```

   `(-1, 0)` means "apply from row 0 to row 0" — i.e. only the header. Every
   data row below it (where the actual Cyrillic words live, e.g. in the
   "Top 20 Words" table) kept using ReportLab's default Helvetica, which is
   why the *Topics* section (built from `Paragraph`, which did have the
   correct font) rendered fine while the *word frequency table* showed
   tofu boxes.

   Fix: extend the range to the whole table body:

   ```python
   ('FONTNAME', (0, 0), (-1, -1), font_name),   # (-1, -1) = every row
   ```

   Applied to all three tables in the PDF report (statistics, word
   frequency, sentiment).

Issue 6: `[nltk_data] Error loading stopwords_bulgarian: Package not found`
(and the same for spanish/french/german/italian/portuguese/dutch/russian/
arabic/turkish)

NLTK doesn't have separate `stopwords_<language>` packages — there's a
single `stopwords` package that already bundles every language NLTK
supports. The old setup script tried to download per-language packages
that don't exist on the NLTK index, which is where those errors came from.

Worse, NLTK's `stopwords` corpus **doesn't include Bulgarian at all**
(nor Polish, Czech, Slovak, Estonian, Latvian, Lithuanian) — so even
without the download errors, those languages were silently falling back
to English stopwords or an empty set.

Fix:
- Remove the bogus per-language download loop; `nltk.download('stopwords')`
  is all that's needed.
- Add `stopwordsiso` as a second stopword source (already listed in
  `requirements.txt` above), which does cover Bulgarian, Polish, Czech,
  Slovak, Estonian, Latvian and Lithuanian. Merge NLTK + stopwordsiso
  results per language, and only fall back to English as a last resort.
- Note that NLTK's fileid for Slovenian is `'slovene'`, not `'slovenian'` —
  the language config table should use that exact name or the lookup
  will silently miss it too.
