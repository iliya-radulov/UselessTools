...
# app_final_complete.py - Updated PDF generation with proper encoding

# Replace the generate_pdf_report function with this updated version
...
def generate_pdf_report(text, results, filename):
    ...
    try:
        ...
        # Try to find a font that supports Cyrillic and other scripts
        try:
            # Try to use a system font that supports Unicode
            # On macOS, use Arial Unicode MS
            # On Windows, use Arial
            # On Linux, use DejaVu Sans
            ...
            for font_path in font_paths:
                try:
                    if os.path.exists(font_path):
                        ...
                        addMapping('UnicodeFont', 0, 0, 'UnicodeFont')
            ...
            if not font_registered:
                # Fallback: Use default font (might not support all characters)
                font_name = 'Helvetica'
            else:
                font_name = 'UnicodeFont'
                
        except:
            # If font registration fails, use default
            font_name = 'Helvetica'
...
# # PDF Report generation - FIXED with proper import handling
# def generate_pdf_report(text, results, filename):
...
#         # Title - FIXED: Use colors properly
#         title_style = ParagraphStyle(
#             'CustomTitle',
#             parent=styles['Title'],
#             fontSize=24,
#             textColor=colors.blue,  # This should work if reportlab is properly imported
#             spaceAfter=30
#         )
#         story.append(Paragraph("Book Analysis Report", title_style))
#         story.append(Spacer(1, 12))
...
st.sidebar.markdown("---")
st.sidebar.markdown("**Version:** 4.2 | Made with ❤️")
