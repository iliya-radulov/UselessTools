# app_final_fixed.py
# Complete Book Analyzer with all fixes
...
# Try to import reportlab, but handle if not available
try:
    ...
    st.warning("ReportLab not installed. PDF export will be disabled.")
...
def download_nltk_data():
    ...
    try:
        ...
        for lang in ['spanish', 'french', 'german', 'italian', 'portuguese', 
                     'dutch', 'russian', 'arabic', 'turkish', 'bulgarian']:
            try:
...
# File readers - FIXED for Bulgarian and better PDF handling
def read_txt(file):
    try:
    ...
    try:
        ...
        for i, page in enumerate(pdf_reader.pages):
            try:
                ...
                st.warning(f"Could not read page {i+1}: {e}")
                continue
        
        # If we got very little text, try a different approach
        if len(text.strip()) < 100 and total_pages > 1:
            st.info("Attempting alternative PDF extraction...")
            # Some PDFs need a different approach
            try:
                text = ""
...
def analyze_text(text, language='English', custom_stopwords=None, num_topics=3):
    ...
    # Keep letters from all languages (including Cyrillic, Arabic, etc.)
    clean_text = re.sub(r'[^a-zA-Zа-яА-ЯёЁъЪюЮяЯіІїЇєЄґҐ\s]', ' ', clean_text)
    
    # Tokenize - use word_tokenize which handles non-English better
    tokens = word_tokenize(clean_text)
    tokens = [word for word in tokens if word not in stop_words]
    tokens = [word for word in tokens if len(word) >= 2]  # Allow 2-letter words for non-English
    
    # Word frequency
    fdist = nltk.FreqDist(tokens)
    
    # Get sentences for sentiment - improved split for non-English
    # Split on common sentence endings in multiple languages
    sentence_endings = r'[.!?…।।॥？！]'
    sentences = [s.strip() for s in re.split(sentence_endings, text) if len(s.strip()) > 5]
    
    # Sentiment analysis (VADER works best for English)
    sentiment_results = []
    if sentences and language == 'English':
        ...
        # For non-English, do a simple sentiment based on word lists (basic)
        # This is a fallback - better to use language-specific sentiment models
    ...
    # Readability - works for any language but best for English
    readability = {}
    if sentences and tokens:
        try:
            ...
            total_syllables = sum(count_syllables(w) for w in tokens[:1000])  # Limit for performance
...
def generate_pdf_report(text, results, filename):
    """Generate PDF report with fixed color issue"""
    ...
    title_style = ParagraphStyle(
        ...
        parent=styles['Title'],
        fontSize=24,
        textColor=colors.HexColor('#1f77b4'),  # FIXED: Using HexColor instead of colors.blue
        spaceAfter=30
    )
    ...
    # Top words
    if results['fdist']:
        story.append(Paragraph("Top 20 Words", styles['Heading2']))
        top_words = results['fdist'].most_common(20)
        word_data = [['Word', 'Frequency']] + [[w, str(f)] for w, f in top_words]
    ...
    # Topics
    if results['topics']:
        story.append(Paragraph("Topics Discovered", styles['Heading2']))
        for i, topic in enumerate(results['topics']):
            topic_text = ', '.join(topic[:10])
            story.append(Paragraph(f"Topic {i+1}: {topic_text}", styles['Normal']))
...
with st.sidebar.expander("📤 Export", expanded=False):
    ...
    # Show warning if PDF not available
...
st.sidebar.markdown("---")
st.sidebar.markdown("**Version:** 4.1 | Made with ❤️")
...
if uploaded_file or direct_text:
    ...
    with st.spinner("📖 Loading your document..."):
        if uploaded_file:
            ...
            # Show file info
            file_size = len(text)
            file_pages = len([p for p in text.split('\n\n') if p.strip()])
            st.info(f"📄 File: {filename} | Size: {file_size:,} characters | ~{file_pages} paragraphs")
    ...
    - 📑 PDF - Documents (improved for non-English)
    - 📝 DOCX - Word documents
    - 📚 EPUB - E-books
    """)
